...
function App() {
  ...
  const startQuiz = () => {
    setStarted(true); setCompleted(false); setIndex(0); setSelected(null); setAnswered(false); setCertificate(null);
    setTimeout(() => quizRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80);
  };

  const checkAnswer = () => {
    if (selected === null) return;
    if (selected === q.correct) { setAnswered(true); setFeedback('correct'); }
    else { setFeedback('wrong'); }
  };

  const retry = () => { setFeedback(null); setSelected(null); };
  const next = () => {
    setFeedback(null);
  ...
  const submitCertificate = async (event) => {
    ...
    try {
      const response = await fetch('/api/certificates', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, language: lang, score: questions.length, total: questions.length }) });
  ...
  return <div className="app">
    ...
    <main>
      <section id="home" className="hero shell">
        ...
        <div className="hero-visual" aria-hidden="true">
          ...
          <div className="hero-card card-back"><span>10</span><small>{lang === 'bg' ? 'КРАТКИ ВЪПРОСА' : 'FOCUSED QUESTIONS'}</small></div>
      ...
      <section id="quiz" ref={quizRef} className={`quiz-section ${started ? 'active' : ''}`}>
        <div className="shell">
          {!started && <div className="quiz-invite"><span>{lang === 'bg' ? 'Готови ли сте?' : 'Ready when you are.'}</span><button className="primary" onClick={startQuiz}>{t.start}<ArrowRight size={18}/></button></div>}
          {started && !completed && <div className="quiz-layout">
            ...
            <article className="question-card" key={q.id}>
              <div className="question-top"><span>{t.choose}</span><div className="mobile-count">{index + 1}/{questions.length}</div></div>
              <h2>{q.text[lang]}</h2>
              <div className="options" role="radiogroup">
                {q.options[lang].map((option, optionIndex) => <button key={option} role="radio" aria-checked={selected === optionIndex} disabled={answered} className={`option ${selected === optionIndex ? 'selected' : ''} ${answered && selected === optionIndex ? 'correct-option' : ''}`} onClick={() => setSelected(optionIndex)}><span className="letter">{String.fromCharCode(65 + optionIndex)}</span><span>{option}</span>{answered && selected === optionIndex && <Check className="option-check" size={19}/>}</button>)}
              </div>
              <div className="question-actions">
                {!answered ? <button className="primary" disabled={selected === null} onClick={checkAnswer}>{t.check}<ChevronRight size={18}/></button> : <><div className="success-inline"><Check size={17}/><span>{t.correct}</span></div><button className="primary" onClick={next}>{index === questions.length - 1 ? t.finish : t.next}<ArrowRight size={18}/></button></>}
              </div>
          ...
          {started && completed && <div className="completion">
            <div className="completion-copy"><div className="seal"><ShieldCheck size={32}/></div><div className="eyebrow"><span></span>{t.completeEyebrow}</div><h2>{t.completeTitle}</h2><p>{t.completeText}</p><div className="result-stat"><strong>{questions.length}/{questions.length}</strong><span>{lang === 'bg' ? 'правилни отговора' : 'correct answers'}</span></div></div>
      ...
      <section id="about" className="about shell"><div><div className="eyebrow"><span></span>{t.about}</div><h2>{t.aboutTitle}</h2></div><div><p>{t.aboutText}</p><div className="pill-row"><span>10 {lang === 'bg' ? 'въпроса' : 'questions'}</span><span>2 {lang === 'bg' ? 'езика' : 'languages'}</span><span>1 {lang === 'bg' ? 'сертификат' : 'certificate'}</span></div></div></section>
    ...
    {feedback && <div className="modal-layer" role="dialog" aria-modal="true">
      <div className={`feedback-modal ${feedback}`}>
        ...
        {feedback === 'wrong' && <><h3>{q.text[lang]}</h3><div className="answer-review"><small>{t.yourAnswer}</small><p>{q.options[lang][selected]}</p></div><div className="explanation"><small>{t.explanation}</small><p>{q.explanation[lang]}</p></div><button className="primary full" onClick={retry}>{t.understand}<ArrowRight size={18}/></button></>}
